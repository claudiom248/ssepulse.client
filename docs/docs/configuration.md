...
| `HandlerFailureBehavior`       | `HandlerFailureBehavior`      | `SkipAndAdvance`                                                         | What happens when an event handler throws: `SkipAndAdvance` stores the event id and goes on, `StopSource` faults the source and does not store the id. See [Last-Event-ID Resumption](last-event-id.md#when-a-handler-throws). |
