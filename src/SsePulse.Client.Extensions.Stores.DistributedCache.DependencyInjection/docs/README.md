...
**SsePulse.Client.Extensions.Stores.DistributedCache.DependencyInjection** bridges `SsePulse.Client.Extensions.Stores.DistributedCache` and `SsePulse.Client.DependencyInjection`. It extends `ISseSourceBuilder` with two `AddDistributedCacheLastEventIdStore()` overloads: resolve an already-registered `IDistributedCache` from the DI container, or provide a custom factory delegate for full control. Both overloads accept an `Action<DistributedCacheLastEventIdStoreOptions>` delegate for configuring the cache key. The `DistributedCacheLastEventIdStore` is registered as a keyed transient service scoped to the named SSE source so multiple sources can each maintain their own independent cache entry.
